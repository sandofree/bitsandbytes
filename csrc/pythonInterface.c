...
void quantkbit_half(half* A, float *absmax, long long *out, int ld, int num_elements, int k){ quantkbit<half>(A, absmax, out, ld, num_elements, k); }
void dequantkbit_half(long long* A, float *absmax, half *out, int ld, int num_elements, int k){ dequantkbit<half>(A, absmax, out, ld, num_elements, k); }
...
extern "C"
{
  ...
  { spmm_coo((cusparseHandle_t) context->m_handle, A_rowidx, A_colidx, A_vals, A_nnz, A_rows, A_cols, B_cols, ldb, B, ldc, C, transposed_B); }
	...
	void cquantkbit_half(half* A, float *absmax, long long *out, int ld, int num_elements, int k){ quantkbit_half(A, absmax, out, ld, num_elements, k); }
	void cdequantkbit_half(long long* A, float *absmax, half *out, int ld, int num_elements, int k){ dequantkbit_half(A, absmax, out, ld, num_elements, k); }
